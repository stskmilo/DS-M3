/*Nombres de campos*/
use henry_m3;
ALTER TABLE `calendario` CHANGE `ID` `IdFecha` INTEGER NOT NULL;
ALTER TABLE `cliente` CHANGE `ID` `IdCliente` INTEGER NOT NULL;
ALTER TABLE `empleado` CHANGE `IDEmpleado` `IdEmpleado` INTEGER;
ALTER TABLE `proveedor` CHANGE `IDProveedor` `IdProveedor` INTEGER NULL DEFAULT NULL;
ALTER TABLE `sucursal` CHANGE `ID` `IdSucursal` INTEGER NULL DEFAULT NULL;
ALTER TABLE `tipo_gasto` CHANGE `Descripcion` `Tipo_Gasto` VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL;
ALTER TABLE `producto` CHANGE `IDProducto` `IdProducto` INTEGER NULL DEFAULT NULL;
